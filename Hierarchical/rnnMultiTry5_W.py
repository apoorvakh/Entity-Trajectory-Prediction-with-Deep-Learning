...
#exit(0)
import pickle
from singleRNN import RNN
rnns = dict()
rnns['Pedestrian'] = pickle.load( open( "ped.pk1", "rb" ) )
rnns['Car'] = pickle.load( open( "car.pk1", "rb" ) )
rnns['Biker'] = pickle.load( open( "biker.pk1", "rb" ) )
rnns['Skater'] = pickle.load( open( "skater.pk1", "rb" ) )
rnns['Cart'] = pickle.load( open( "cart.pk1", "rb" ) )
#print rnns['Pedestrian'].Wxh
#exit(0)

for c in wtMatrices:
    #print c
    d = dict()
    d['Wxh'] = rnns[c].Wxh #np.random.randn(hidden_size, vocab_size) * 0.3  # input to hidden
    d['Whh'] = rnns[c].Whh #np.random.randn(hidden_size, hidden_size) * 0.3  # hidden to hidden
    d['Why'] = rnns[c].Why #np.random.randn(vocab_size, hidden_size) * 0.3  # hidden to output
    d['bh'] = rnns[c].bh #np.random.randn(hidden_size, 1) * 0.3  # hidden bias
    d['by'] = rnns[c].by #np.random.randn(vocab_size, 1) * 0.3  # output bias
...
class RNN():
    ...
    def lossFunc(self, inputs, eClass, Ht, targets, frameNo, entityNo, hprev):
        ...
        for t in xrange(len(inputs)):
            ...
            #print wtMatrices[eClass]['Wxh'].shape, xs[t].shape, wtMatrices[eClass]['Whh'].shape, hs[t-1].shape, wtMatrices[eClass]['bh'].shape

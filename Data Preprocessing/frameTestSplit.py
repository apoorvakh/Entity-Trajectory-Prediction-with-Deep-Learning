...
frameTestDestination="C:\\Anju\\Final Year Project\\Stanford drone dataset\\newannotations\\test\\frame"
...
def threadFunc(files):
    for file in files:
        ...
        y=re.match('(.*)\\\\(.*).csv',file)
        name=y.groups()[1]
        os.makedirs(frameTestDestination+'\\'+name)
        readerIterator=reader.__iter__()
        sortedlist = sorted(readerIterator, key=operator.itemgetter(7))

        for item in sortedlist:
            file=open(frameTestDestination+'\\'+name+'\\'+name+'_'+str(item[7])+'.csv','a',newline='')
